# NC News Backend

A REST API which allows CRUD requests for data for a forum.

## Description

Link to the hosted API: https://rickys-nc-news-be.onrender.com/ \
(It might take up to a minute to activate as the free tier of Render spins down after 15 minutes of inactivity.)

This is a Backend Solo Project to build a a Forum API. There are preset topics that have can have multiple articles for each topic. And under each article users can leave comments in the article and upvote or downvote the article.

## Test Setup guide

### Dependencies

Jest, pg, pg-format, dotenv, express, supertest and jest extended

### Setup procedures

1. Set up the .env file for tests. It is required for dotenv to setup the config for connection to the database.

- .env.test file is required for specifying database for test.

2. Install NPM packages.

```
npm install
```

3. Set up local databases

```
npm run setup-dbs
```

4. Run test files

```
npm t
```

The file will be accessed by connection.js through pg method.

## Dev Setup guide

1. Set up the .env file for development. It is required for dotenv to setup the config for connection to the database.

- .env.development is required for specifying database for local development.

2. Install NPM packages.

```
npm install
```

3. Set up local databases

```
npm run setup-dbs
```
4. Seed the tables and insert development data to the tables

```
npm run seed-dev
```


